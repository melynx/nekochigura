...
# drm/xe#7513: xe allocates the display page table from stolen memory on
# non-DGFX parts, which hard-freezes LNL under GPU load. Still unfixed in
# 7.1.x -- 7.1.9 has no drm/xe commits at all and 7.1.10's leave xe_fb_pin.c
# byte-identical to v7.1.8's -- so the rebase made for 7.1.5 applies here
# unchanged and keeps its name.
PATCHES=(
	"${FILESDIR}/${PN}-7.1.5-xe-dpt-no-stolen.patch"
)
