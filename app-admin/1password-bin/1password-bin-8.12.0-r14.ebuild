...
MY_PN="${PN/-bin/}"
MY_PV="${PV%%-r*}"  # Strip revision to get base version
CLI_PV="2.32.0"

SRC_URI="
	amd64? ( https://downloads.1password.com/linux/tar/beta/x86_64/${MY_PN}-${MY_PV}-14.BETA.x64.tar.gz -> ${P}-amd64.tar.gz )
	arm64? ( https://downloads.1password.com/linux/tar/beta/aarch64/${MY_PN}-${MY_PV}-14.BETA.arm64.tar.gz -> ${P}-arm64.tar.gz )
	cli? (
...
pkg_postinst() {
	...
	elog ""
	ewarn "This is a BETA version and may contain bugs."
	ewarn "For production use, consider using the stable version instead."
	elog ""
