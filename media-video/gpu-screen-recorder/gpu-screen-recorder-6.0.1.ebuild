...
PATCHES=(
	"${FILESDIR}/${PN}-6.0.1-dbus-without-pipewire.patch"
)
...
src_configure() {
	local emesonargs=(
		...
		# 'auto' builds a bundled ffmpeg from source in release builds; the
		# eclass only avoids that by defaulting to buildtype=plain, so pin it.
