# Copyright 2026 Gentoo Authors
# Distributed under the terms of the GNU General Public License v2

# Autogenerated by pycargoebuild 0.15.1

EAPI=8

CRATES="
	addr2line@0.21.0
	adler2@2.0.1
	adler@1.0.2
	ahash@0.8.11
	aho-corasick@1.1.3
	aligned-vec@0.5.0
	aligned-vec@0.6.4
	aligned@0.4.3
	alloc-no-stdlib@2.0.4
	alloc-stdlib@0.2.2
	allocator-api2@0.2.21
	anes@0.1.6
	anstream@0.6.15
	anstyle-parse@0.2.5
	anstyle-query@1.1.1
	anstyle-wincon@3.0.4
	anstyle@1.0.8
	anyhow@1.0.86
	approx@0.5.1
	arbitrary@1.3.2
	arg_enum_proc_macro@0.3.4
	ariadne@0.5.1
	arrayvec@0.7.6
	as-slice@0.2.1
	autocfg@1.3.0
	av-scenechange@0.14.1
	av1-grain@0.2.3
	avif-serialize@0.8.8
	backtrace@0.3.71
	base64@0.21.7
	base64@0.22.1
	bit_field@0.10.2
	bitflags@1.3.2
	bitflags@2.11.0
	bitstream-io@4.9.0
	block-buffer@0.10.4
	brotli-decompressor@5.0.0
	built@0.8.0
	bumpalo@3.16.0
	by_address@1.2.1
	bytemuck@1.25.0
	byteorder-lite@0.1.0
	byteorder@1.5.0
	bytes@1.7.1
	cast@0.3.0
	cc@1.2.18
	cfg-if@1.0.0
	chumsky@0.10.1
	ciborium-io@0.2.2
	ciborium-ll@0.2.2
	ciborium@0.2.2
	clap@4.5.16
	clap_builder@4.5.15
	clap_derive@4.5.13
	clap_lex@0.7.2
	color-eyre@0.6.5
	color_quant@1.1.0
	colorchoice@1.0.2
	colorsys@0.6.7
	comfy-table@7.2.2
	console@0.16.2
	convert_case@0.10.0
	core-foundation-sys@0.8.7
	core-foundation@0.9.4
	core2@0.4.0
	cpufeatures@0.2.17
	crc32fast@1.4.2
	criterion-plot@0.5.0
	criterion@0.5.1
	crossbeam-deque@0.8.5
	crossbeam-epoch@0.9.18
	crossbeam-utils@0.8.20
	crossterm@0.29.0
	crossterm_winapi@0.9.1
	crunchy@0.2.2
	crypto-common@0.1.6
	csscolorparser@0.7.2
	dialoguer@0.12.0
	digest@0.10.7
	directories@5.0.1
	dirs-sys@0.3.7
	dirs-sys@0.4.1
	dirs@4.0.0
	document-features@0.2.12
	either@1.13.0
	encode_unicode@1.0.0
	encoding_rs@0.8.34
	enquote@1.1.0
	env_logger@0.10.2
	equator-macro@0.4.2
	equator@0.4.2
	equivalent@1.0.1
	errno@0.3.14
	execute-command-macro-impl@0.1.10
	execute-command-macro@0.1.9
	execute-command-tokens@0.1.7
	execute@0.2.15
	exr@1.74.0
	eyre@0.6.12
	fast-srgb8@1.0.0
	fax@0.2.6
	fax_derive@0.2.0
	fdeflate@0.3.4
	flate2@1.1.9
	fnv@1.0.7
	foldhash@0.1.5
	form_urlencoded@1.2.1
	futures-channel@0.3.30
	futures-core@0.3.30
	futures-io@0.3.30
	futures-sink@0.3.30
	futures-task@0.3.30
	futures-util@0.3.30
	generic-array@0.14.7
	generic-array@1.1.0
	getrandom@0.2.15
	getrandom@0.3.4
	gif@0.14.1
	gimli@0.28.1
	h2@0.3.26
	half@2.4.1
	hashbrown@0.15.2
	heck@0.5.0
	hermit-abi@0.3.9
	hermit-abi@0.4.0
	http-body-util@0.1.2
	http-body@0.4.6
	http-body@1.0.1
	http@0.2.12
	http@1.1.0
	httparse@1.9.4
	httpdate@1.0.3
	humantime@2.1.0
	hyper-rustls@0.24.2
	hyper-rustls@0.27.2
	hyper-util@0.1.7
	hyper@0.14.30
	hyper@1.4.1
	idna@0.5.0
	image-webp@0.2.4
	image@0.25.10
	imgref@1.12.0
	indenter@0.3.4
	indexmap@2.7.0
	interpolate_name@0.2.4
	ipnet@2.9.0
	is-terminal@0.4.13
	is_terminal_polyfill@1.70.1
	itertools@0.10.5
	itertools@0.14.0
	itoa@1.0.11
	jobserver@0.1.32
	js-sys@0.3.70
	jxl-bitstream@1.1.0
	jxl-coding@1.0.1
	jxl-color@0.11.0
	jxl-frame@0.13.3
	jxl-grid@0.6.1
	jxl-image@0.13.0
	jxl-jbr@0.2.1
	jxl-modular@0.11.2
	jxl-oxide-common@1.0.0
	jxl-oxide@0.12.5
	jxl-render@0.12.3
	jxl-threadpool@1.0.0
	jxl-vardct@0.11.1
	lebe@0.5.2
	libc@0.2.182
	libfuzzer-sys@0.4.7
	libredox@0.1.3
	linux-raw-sys@0.11.0
	litrs@1.0.0
	lock_api@0.4.14
	log@0.4.22
	loop9@0.1.5
	material-colors@0.4.2
	maybe-rayon@0.1.1
	memchr@2.7.4
	mime@0.3.17
	minimal-lexical@0.2.1
	miniz_oxide@0.7.4
	miniz_oxide@0.8.9
	mio@1.0.2
	moxcms@0.8.1
	new_debug_unreachable@1.0.6
	nom@7.1.3
	noop_proc_macro@0.3.0
	num-bigint@0.4.6
	num-derive@0.4.2
	num-integer@0.1.46
	num-rational@0.4.2
	num-traits@0.2.19
	object@0.32.2
	once_cell@1.19.0
	oorandom@11.1.4
	option-ext@0.2.0
	owo-colors@4.0.0
	palette@0.7.6
	palette_derive@0.7.6
	paris-log@1.0.2
	paris@1.5.15
	parking_lot@0.12.5
	parking_lot_core@0.9.12
	paste@1.0.15
	pastey@0.1.1
	percent-encoding@2.3.1
	phf@0.11.3
	phf_generator@0.11.3
	phf_macros@0.11.3
	phf_shared@0.11.3
	pin-project-internal@1.1.5
	pin-project-lite@0.2.14
	pin-project@1.1.5
	pin-utils@0.1.0
	plotters-backend@0.3.6
	plotters-svg@0.3.6
	plotters@0.3.6
	png@0.18.1
	ppv-lite86@0.2.20
	pretty_env_logger@0.5.0
	proc-macro2@1.0.106
	profiling-procmacros@1.0.16
	profiling@1.0.16
	psm@0.1.25
	pxfm@0.1.28
	qoi@0.4.1
	quick-error@2.0.1
	quinn-proto@0.11.6
	quinn-udp@0.5.4
	quinn@0.11.3
	quote@1.0.36
	r-efi@5.3.0
	rand@0.8.5
	rand@0.9.2
	rand_chacha@0.3.1
	rand_chacha@0.9.0
	rand_core@0.6.4
	rand_core@0.9.5
	rav1e@0.8.1
	ravif@0.13.0
	rayon-core@1.13.0
	rayon@1.11.0
	redox_syscall@0.5.18
	redox_users@0.4.5
	regex-automata@0.3.9
	regex-automata@0.4.9
	regex-syntax@0.7.5
	regex-syntax@0.8.5
	regex@1.11.1
	reqwest@0.11.27
	reqwest@0.12.5
	resolve-path@0.1.0
	rgb@0.8.53
	ring@0.17.8
	rustc-demangle@0.1.24
	rustc-hash@2.0.0
	rustix@1.1.3
	rustls-pemfile@1.0.4
	rustls-pemfile@2.1.3
	rustls-pki-types@1.8.0
	rustls-webpki@0.101.7
	rustls-webpki@0.102.6
	rustls@0.21.12
	rustls@0.23.12
	ryu@1.0.18
	same-file@1.0.6
	scopeguard@1.2.0
	sct@0.7.1
	semver@1.0.23
	serde@1.0.219
	serde_derive@1.0.219
	serde_json@1.0.125
	serde_spanned@0.6.7
	serde_urlencoded@0.7.1
	sha2@0.10.9
	shell-words@1.1.1
	shlex@1.3.0
	simd-adler32@0.3.7
	simd_helpers@0.1.0
	siphasher@1.0.1
	slab@0.4.9
	smallvec@1.13.2
	socket2@0.5.7
	spin@0.9.8
	stable_deref_trait@1.2.1
	stacker@0.1.20
	strsim@0.11.1
	subtle@2.6.1
	syn@2.0.87
	sync_wrapper@0.1.2
	sync_wrapper@1.0.1
	system-configuration-sys@0.5.0
	system-configuration@0.5.1
	termcolor@1.4.1
	thiserror-impl@1.0.63
	thiserror-impl@2.0.12
	thiserror@1.0.63
	thiserror@2.0.12
	tiff@0.11.3
	tinytemplate@1.2.1
	tinyvec@1.8.0
	tinyvec_macros@0.1.1
	tokio-rustls@0.24.1
	tokio-rustls@0.26.0
	tokio-util@0.7.11
	tokio@1.39.3
	toml@0.8.19
	toml_datetime@0.6.8
	toml_edit@0.22.20
	tower-layer@0.3.3
	tower-service@0.3.3
	tower@0.4.13
	tracing-core@0.1.32
	tracing@0.1.40
	try-lock@0.2.5
	typenum@1.17.0
	unicode-bidi@0.3.15
	unicode-ident@1.0.12
	unicode-normalization@0.1.23
	unicode-segmentation@1.12.0
	unicode-width@0.1.13
	unicode-width@0.2.2
	untrusted@0.9.0
	update-informer@1.1.0
	ureq@2.10.1
	url@2.5.2
	utf8parse@0.2.2
	v_frame@0.3.8
	version_check@0.9.5
	walkdir@2.5.0
	want@0.3.1
	wasi@0.11.0+wasi-snapshot-preview1
	wasip2@1.0.2+wasi-0.2.9
	wasm-bindgen-backend@0.2.93
	wasm-bindgen-futures@0.4.43
	wasm-bindgen-macro-support@0.2.93
	wasm-bindgen-macro@0.2.93
	wasm-bindgen-shared@0.2.93
	wasm-bindgen@0.2.93
	web-sys@0.3.70
	webpki-roots@0.25.4
	webpki-roots@0.26.3
	weezl@0.1.12
	winapi-i686-pc-windows-gnu@0.4.0
	winapi-util@0.1.9
	winapi-x86_64-pc-windows-gnu@0.4.0
	winapi@0.3.9
	windows-link@0.2.1
	windows-sys@0.48.0
	windows-sys@0.52.0
	windows-sys@0.59.0
	windows-sys@0.61.2
	windows-targets@0.48.5
	windows-targets@0.52.6
	windows_aarch64_gnullvm@0.48.5
	windows_aarch64_gnullvm@0.52.6
	windows_aarch64_msvc@0.48.5
	windows_aarch64_msvc@0.52.6
	windows_i686_gnu@0.48.5
	windows_i686_gnu@0.52.6
	windows_i686_gnullvm@0.52.6
	windows_i686_msvc@0.48.5
	windows_i686_msvc@0.52.6
	windows_x86_64_gnu@0.48.5
	windows_x86_64_gnu@0.52.6
	windows_x86_64_gnullvm@0.48.5
	windows_x86_64_gnullvm@0.52.6
	windows_x86_64_msvc@0.48.5
	windows_x86_64_msvc@0.52.6
	winnow@0.6.18
	winreg@0.50.0
	winreg@0.52.0
	wit-bindgen@0.51.0
	y4m@0.8.0
	yansi@1.0.1
	zerocopy-derive@0.7.35
	zerocopy@0.7.35
	zeroize@1.8.1
	zune-core@0.5.1
	zune-inflate@0.2.54
	zune-jpeg@0.5.12
"

RUST_MIN_VER="1.88.0"

inherit cargo

DESCRIPTION="Material You and Base16 color generation tool with templates"
HOMEPAGE="https://github.com/InioX/matugen"
SRC_URI="
	https://github.com/InioX/${PN}/archive/v${PV}.tar.gz -> ${P}.gh.tar.gz
	${CARGO_CRATE_URIS}
"

LICENSE="GPL-2+"
# Dependent crate licenses
LICENSE+="
	Apache-2.0 BSD-2 BSD ISC MIT MPL-2.0 Unicode-DFS-2016 Unlicense ZLIB
"
SLOT="0"
KEYWORDS="~amd64"

# No PATCHES: 4.1.0 carried ${PN}-4.1.0-compiler-warnings.patch to silence an
# unused std::process::Stdio import and IndexMap's deprecated remove(). Upstream
# fixed both in 4.2.0, choosing swap_remove() where the patch used shift_remove().
# Context::data is not Serialize and is not iterated for output, so dropping the
# order-preserving variant in favour of upstream's choice is behaviour-neutral here.

src_install() {
	cargo_src_install
	einstalldocs
}
