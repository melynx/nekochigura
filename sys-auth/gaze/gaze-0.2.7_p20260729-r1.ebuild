...
CRATES="
	...
	opencv-binding-generator@0.103.0
	opencv@0.99.1
...
GAZE_COMMIT="1674c3144731068f3964267a47f1ecf5f8f6c867"
...
SRC_URI="
	https://github.com/melynx/gaze/archive/${GAZE_COMMIT}.tar.gz -> ${PF}.tar.gz
	${CARGO_CRATE_URIS}
"
...
SLOT="0"
KEYWORDS="~amd64"
IUSE="+gui +hyprlock openvino test"
RESTRICT="!test? ( test )"
...
RDEPEND="
	...
	openvino? ( >=sci-libs/onnxruntime-1.27.1-r1:=[openvino] )
	!openvino? ( sci-libs/onnxruntime:= )
	sys-apps/dbus
	sys-auth/polkit
	sys-libs/pam
	gui? (
...
PATCHES=(
	"${FILESDIR}/${P}-contain-pam-panics.patch"
	"${FILESDIR}/${P}-report-frame-processing-errors.patch"
)
...
src_compile() {
	...
	local -a daemon_args=( --package gaze )
	use openvino && daemon_args+=( --features gaze/openvino )
	cargo_src_compile "${daemon_args[@]}"

	local -a client_args=(
		--package gaze-cli
		--package pam-gaze
	)
	use gui && client_args+=( --package gaze-gui )
	if use openvino; then
		local client_features="gaze-cli/openvino"
		use gui && client_features+=",gaze-gui/openvino"
		client_args+=( --features "${client_features}" )
	fi
	cargo_src_compile "${client_args[@]}"
}
...
src_test() {
	...
	for package in "${packages[@]}"; do
		local -a cargo_args=( --package "${package}" )
		if use openvino; then
			case "${package}" in
				gaze)
					cargo_args+=( --features gaze/openvino )
					;;
				gaze-cli)
					cargo_args+=( --features gaze-cli/openvino )
					;;
				gaze-core)
					cargo_args+=( --features gaze-core/openvino )
					;;
				gaze-gui)
					cargo_args+=( --features gaze-gui/openvino )
					;;
			esac
		fi
		cargo_src_test "${cargo_args[@]}"
	done
}
