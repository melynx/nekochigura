...
### Issue 58 — Space Grotesk old style-guide build cleanup

Status: fixed, verified, signed, and published on 2026-07-20 in commit
`9c564896b8e3e12bc8143a6bc22a4b288cbe1c3e`.

- Checked every tag in the official `floriankarsten/space-grotesk` repository.
  Version 2.0.0 remains the latest release, at commit `7220f5d`. Kept its
  official source ebuild and removed shadowed version 1.1.4-r1. The removed
  ebuild used the separate 38C3 2024 style-guide archive rather than the
  official Space Grotesk release source.
- Regenerated the Manifest. It now contains only the 2.0.0 official source
  archive and no longer asks users to download the 31.8 MB 38C3 style-guide
  bundle. Space Grotesk is not installed on this laptop, and no checked user
  configuration selects it, so the cleanup has no live effect.
- A clean network-sandboxed build and staged install pass. Upstream supplies no
  package test suite. The image contains the official Regular, Light, Medium,
  and Bold OTF fonts, four compressed upstream documents, and the three font
  index files generated by the Gentoo font eclass.
- Each staged font matches its upstream OTF byte for byte. Fontconfig parses
  all four as static CFF outline fonts in the Space Grotesk family and reports
  the expected styles. All fonts have the correct OpenType signature. The
  compressed OFL license, README, author list, and contributor list reproduce
  their upstream files exactly.
- Every staged file has mode 0644. The image has no executable or
  world-writable file and no broken link. Syntax, Manifest, whitespace,
  source-comparison, document, license, font-signature, Fontconfig, staged-tree,
  and targeted package checks pass. The fresh full non-network overlay scan
