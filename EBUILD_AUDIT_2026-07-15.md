...
### SongRec 0.7.4 vendor bundle — generated and published

With maintainer approval, the official SongRec 0.7.4 source was downloaded and
`cargo vendor --locked --versioned-dirs vendor` was run using Cargo 1.96.1.

Generated file:

```text
nekochigura-dependencies/app-misc/songrec/songrec-0.7.4-vendor.tar.xz
```

Current bundle facts:

- Compressed size: 39,122,840 bytes (approximately 38 MiB).
- SHA-256:
  `0b18d648ff1d2e6dff30ec93450b7298f682238d0262b47c5240c2724ef10ae7`
- 345 versioned crate directories.
- 22,944 archive entries.
- Contains only `vendor/`, including crate checksum and license files. Cargo's
  configuration is deliberately generated by `cargo.eclass` in the ebuild;
  placing `.cargo/config.toml` in the archive caused duplicate source entries.
- Archive integrity (`xz -t`) and path traversal checks passed.
- `Cargo.lock` contains no Git sources in 0.7.4. CPAL is now crates.io
  `cpal-0.18.1`, with the PipeWire feature enabled on Linux. The earlier audit
  statement that CPAL was a Git dependency applied to the previous assumption
  and is not true for 0.7.4.
- `cargo metadata --locked --offline` succeeded against the original generated
  vendor tree.
- A fresh copy of the official source plus a fresh extraction of the compressed
  archive passed `cargo metadata --locked --offline`; locked offline dependency
  resolution is verified from the actual archive.
- The archive was committed and pushed to the dependency repository's `master`
  branch as `dc3c6ac5728ca85ac7d20d4b8cfe94eb433c6487`. The parent ebuild's raw
  GitHub `SRC_URI` can therefore fetch the published bundle.
...
- Added `songrec-0.7.4.ebuild`, using `cargo.eclass` and a single external
  vendor archive for a locked, network-free build.
...
- `pkgcheck` reported no findings for the new ebuild before pruning or for the
  ...
  also passed.
- A clean Portage run resolved all 345 crates from the extracted vendor archive
  without network access and completed the optimized build in 1m27s. The
  staged install then succeeded with an unprivileged image owner, produced a
  15.8 MiB installed tree, and stripped the PIE executable successfully.
- The generated archive was committed and pushed first in the dependency
  repository as `dc3c6ac`; the parent change records that gitlink together with
  `.gitmodules`, the ebuild, metadata, Manifest, pruning, and this ledger.
