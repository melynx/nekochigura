...
src_unpack() {
	...
	tar -xzf "${DISTDIR}/${A}" "${HIPSPARSELT}" "${HIPBLASLT}" "${ORIGAMI}" -C "${WORKDIR}" || die
}

src_prepare() {
	local TENSILELITE_DIR="${WORKDIR}/rocm-libraries-rocm-${PV}/projects/hipblaslt/tensilelite"
	local shebangs=($(grep -rl "#!/usr/bin/env python3" "${TENSILELITE_DIR}/Tensile" || die))
	python_fix_shebang -q "${shebangs[@]}"

	rocm_use_clang

	sed -e "s:\$(ROCM_PATH)/bin/amdclang++:$(get_llvm_prefix)/bin/clang++:g" \
		-i "${TENSILELITE_DIR}/Makefile" || die

	# Fix compiler validation (just a validation)
	sed -e "s/amdclang/$(basename "$CC")/g" \
		-i "${TENSILELITE_DIR}/Tensile/Toolchain/Validators.py" \
		-i "${TENSILELITE_DIR}/Tensile/Tests/unit/test_MatrixInstructionConversion.py" || die
...
src_configure() {
	...
	local mycmakeargs=(
		...
		-DHIPSPARSELT_ENABLE_CLIENT="$(usex benchmark ON $(usex test ON OFF))"
		...
		-DHIPSPARSELT_BUNDLE_PYTHON_DEPS=ON
	...
	if use test || use benchmark; then
		# HIPSPARSELT_ENABLE_CLIENT=ON branch
		mycmakeargs+=(
...
src_test() {
	...
	# Similar test pattern to hipBLASLt
