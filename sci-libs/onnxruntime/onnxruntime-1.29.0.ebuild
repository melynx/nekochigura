...
"
SRC_URI="
	https://github.com/microsoft/onnxruntime/archive/refs/tags/v${PV}.tar.gz -> ${PN}-${PV}.tar.gz
...
# OpenVINO is loaded with dlopen() as an ONNX Runtime execution-provider plugin.
QA_SONAME="usr/lib.*/libonnxruntime_providers_openvino.so"
...
RDEPEND="
	...
	openvino? ( sci-libs/openvino-bin:= )
...
PATCHES=(
	...
	"${FILESDIR}/${PN}-1.29.0-use-system-libraries.patch"
)
...
src_configure() {
	...
	local mycmakeargs=(
		...
		-Donnxruntime_DISABLE_RTTI=$(usex openvino OFF ON)
		-Donnxruntime_USE_OPENVINO=$(usex openvino)
		-Donnxruntime_USE_OPENVINO_CPU=$(usex openvino)
		-Donnxruntime_USE_FULL_PROTOBUF=$(usex openvino)
